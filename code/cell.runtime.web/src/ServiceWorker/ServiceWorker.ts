...
export const ServiceWorker = {
   ...
   */
  start(path: string, options: { localhost?: boolean } = {}) {
    type T = { started: boolean; path: string; error?: string };
    return new Promise<T>(async (resolve, reject) => {
      ...
      if (location.hostname === 'localhost' && !options.localhost) {
      ...
      try {
        ...
        if (!registration.installing) {
          return done(false, `Service worker not activated BUT not installing.`);
        }
