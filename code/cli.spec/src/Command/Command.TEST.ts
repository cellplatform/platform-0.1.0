...
describe('Command', () => {
  it('minimal construction', () => {
    const cmd = Command.create({ title: '  Foo  ' });
  ...
  it('takes handler in constructor', () => {
    const handler = () => true;
    const cmd = Command.create({ title: 'Foo', handler });
    expect(cmd.handler).to.eql(handler);
  });

  it('takes children in constructor', () => {
    const child = Command.create({ title: 'child' });
    const parent = Command.create({ title: 'parent', children: [child] });
    expect(parent.children).to.eql([child]);
  });

  it('throws if a title is not passed', () => {
    const test = (title?: any) => {
      const fn = () => Command.create({ title, handler: () => null });
      expect(fn).to.throw();
    };
    test('');
    test('  ');
  ...
  describe('clone', () => {
    it('clones - deep (default)', () => {
      const child = Command.create({ title: 'child' });
      const cmd1 = Command.create({ title: 'foo', children: [child] });
