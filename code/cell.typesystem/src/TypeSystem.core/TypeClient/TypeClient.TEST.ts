...
describe('TypeClient', () => {
  ...
  describe('errors', () => {
    ...
    it('error: 404 type definition in column reference not found', async () => {
      ...
      const res = await TypeClient.load({ ns: 'foo', fetch: stub.fetch({ defs }) });
    ...
    it('error: ref has invalid property-name', async () => {
      ...
      const res = await TypeClient.load({ ns: 'foo.1', fetch: stub.fetch({ defs }) });
    ...
    it('error: duplicate property names', async () => {
      ...
      const res = await TypeClient.load({ ns: 'foo.error', fetch: stub.fetch({ defs }) });
    ...
    it('error: namespace typename invalid (eg "foo", ".foo", "foo-1")', async () => {
      const test = async (typename: string) => {
        ...
        const ns = 'ns:foo';
        const res = await TypeClient.load({ ns, fetch: stub.fetch({ defs }) });
        const errors = res.errors;
  ...
  describe('types', () => {
    ...
    describe('REF(ns) - object', () => {
      ...
      it('REF(ns) - default/optional retrieved', async () => {
        ...
        const def = (await TypeClient.load({ ns: 'foo.1', fetch: stub.fetch({ defs }) })).defs[0];
        const A = def.columns[0];
    ...
    describe('REF(column) => <uri>', () => {
      ...
      it('REF(column) => VALUE (primitive): multi-hop', async () => {
        const res1 = await TypeClient.load({ ns: 'foo.1', fetch: stub.fetch({ defs }) });
        const res2 = await TypeClient.load({ ns: 'foo.2', fetch: stub.fetch({ defs }) });
  ...
  describe('cache', () => {
    ...
    describe('fetch', () => {
      ...
      it('default cache (within call)', async () => {
        ...
        expect(fetch.count.getNs).to.eql(3); // NB: Less that the total number of type lookups passed to fetch.

        // NB: Call count increases as a shared cache was not given.
        await TypeClient.load({ ns, fetch });
        expect(fetch.count.getNs).to.eql(6);

        await TypeClient.load({ ns, fetch });
        expect(fetch.count.getNs).to.eql(9);
      });
      ...
      it('shared cache (between calls)', async () => {
        ...
        expect(fetch.count.getNs).to.eql(3);
        ...
        expect(fetch.count.getNs).to.eql(3); // NB: Does not increase. Values retrieved from shared cache.
      });
    });
  });
