...
describe('TypeCache', () => {
  ...
  describe('TypeCacheCells', () => {
    ...
    describe('query', () => {
      ...
      it('query.get (pulls full rows, adds to cells)', async () => {
        const entry = TypeCacheCells.create('ns:foo');
        expect(entry.cells).to.eql({});
      ...
      it('query.get (cached results returned)', async () => {
        ...
        expect(count).to.eql(1); // NB: fetch count not increased because cached cells returned.
        ...
        expect(count).to.eql(2); // NB: fetch called again
