...
describe('cell/files: delete, unlink', () => {
  it('delete and/or unlink files from cell', async () => {
    ...
    const res1 = await clientA1.fs.list();
    ...
    const res2 = await clientA1.fs.delete(['', '  ']); // NB: All values collapse to nothing.
    ...
    // Delete single file.
    const res3 = await clientA1.fs.delete('cat&bird.png');
    ...
    await expectFileInFs(res1.body[3].uri, true);
    expect((await clientA1.fs.list()).body.length).to.eql(3);
    ...
    const res4 = await clientA1.fs.delete(['func.wasm', 'foo.json']);
    ...
    expect((await clientA1.fs.list()).body.length).to.eql(1);

    // Report error for a delete request that is not linked to the cell.
    const res5 = await clientA1.fs.delete('not-linked.pdf');
  ...
  it('unlink files from cell', async () => {
    ...
    // Unlink (one).
    const res2 = await clientA1.fs.unlink('kitten.jpg');
    expect(res2.body.deleted).to.eql([]);
    expect(res2.body.unlinked).to.eql(['kitten.jpg']);
    expect(res2.body.errors).to.eql([]);

    const res3 = await clientA1.fs.unlink(['foo.json', 'func.wasm']);
    ...
    await expectFileInFs(res1.body[1].uri, true);
    await expectFileInFs(res1.body[2].uri, true);
    expect((await clientA1.fs.list()).body.length).to.eql(0);
  ...
  it('fails when unlinking a file that is not referenced by the cell', async () => {
    ...
    await clientA1.fs.upload([{ filename: 'func.wasm', data }]);

    const res = await clientA1.fs.unlink('kitten.jpg');
