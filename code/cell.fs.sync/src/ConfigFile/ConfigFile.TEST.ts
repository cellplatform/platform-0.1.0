...
const VALID: t.IConfigFileData = { host: 'domain.com', target: 'cell:foo:A1' };
const PATH = {
...
describe('ConfigDir', () => {
  ...
  it('uses given directory path', () => {
    ...
    const res = ConfigFile.create({ dir });
  ...
  it('target.url', async () => {
    ...
    expect(url).to.eql('https://domain.com/cell:foo:A1/fs');
  });

  describe('load', () => {
    it('does not know if file exists (load not called)', () => {
      const config = ConfigFile.create({ dir: PATH.TMP });
      expect(config.exists).to.eql(null);
    });

    it('does not exist', async () => {
      const config = await ConfigFile.load({ dir: PATH.TMP });
      expect(config.exists).to.eql(false);
    });

    it('load existing data', async () => {
      const config1 = await ConfigFile.load({ dir: PATH.TMP });
      ...
      const config2 = await ConfigFile.load({ dir: PATH.TMP });
  ...
  describe('validation', () => {
    ...
    it('invalid', async () => {
      const test = async (modify: (data: t.IConfigFileData) => void, error: string) => {
        const config = await ConfigFile.create({ dir: PATH.TMP }).save(VALID);
        modify(config.data);
        const res = config.validate();
