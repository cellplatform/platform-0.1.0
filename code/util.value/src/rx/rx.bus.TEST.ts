...
describe('rx.bus', () => {
  ...
  describe('bus.clone', () => {
    type E = { type: 'foo'; payload: { count: number; msg?: string } };
    ...
    it('config: same boundary filter isolates clones', () => {
      const source = rx.bus<E>();
      const target = rx.bus.clone<E>(source, ($) =>
        $.pipe(filter((e) => e.payload.msg !== 'hello')),
      );
      ...
      source.fire({ type: 'foo', payload: { count: 0, msg: 'hello' } });

      expect(fired.source.length).to.eql(1);
      expect(fired.target.length).to.eql(0);

      fired = { source: [], target: [] };
      target.fire({ type: 'foo', payload: { count: 0, msg: 'hello' } });
      expect(fired.source.length).to.eql(0);
      expect(fired.target.length).to.eql(1);
    });
